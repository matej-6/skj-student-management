...
export class StudentComponent implements OnInit {
  ...
  resetEdit() {
    ...
    this.model.datum_narodenia = this.student().datum_narodenia
      ? new Date(this.student().datum_narodenia as Date)
          .toISOString()
          .substring(0, 10)
      : undefined;
    ...
    this.model.priemer = this.student().priemer
      ? Number(this.student().priemer)
      : undefined;
  }
  ...
  handleEdit() {
    ...
    if (
      this.model.priemer &&
      (this.model.priemer < 1 || this.model.priemer > 5)
    ) {
